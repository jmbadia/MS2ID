...
The resulting variable (e.g. `annotResult`) is an Annot object (see  [proper section](#annot-object)).
...
annotResult <- annotate(QRYdir = queryDir, MS2ID = refLibrary, nsamples = 100,
                        ...
                        cmnTopPeaks = 4, cmnPolarity = TRUE, db = "MassBank",
                        nature = "experimental")
```

Other arguments subset the reference library according its nature, the primal database or the polarization of the reference spectra (`nature`, `db` and `cmnPolarity` respectively).

## Annot object
This object stores the annotation results so we can extract particular content by appplyng them *getters* (custom functions to *get* the info) or save the whole object for further analysis (use saveRDS() and loadRDS()).
